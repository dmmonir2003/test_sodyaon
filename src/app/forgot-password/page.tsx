...
import { ArrowRight, Package, ShieldCheck, Mail, Lock, CheckCircle2 } from "lucide-react";
import { 
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
      <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-slate-700">
        ...
        <div className="bg-primary-600 p-8 text-center text-white relative">
          ...
          <h2 className="text-2xl font-black font-heading relative z-10">পাসওয়ার্ড রিকভারি</h2>
          <p className="text-primary-100 text-sm mt-2 relative z-10">আপনার একাউন্টের অ্যাক্সেস ফিরে পান</p>
        </div>
        ...
        <div className="p-8">
          ...
          <div className="flex items-center justify-center gap-2 mb-8">
            <div className={`w-2.5 h-2.5 rounded-full ${step >= 1 ? 'bg-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
            <div className={`w-8 h-1 rounded-full ${step >= 2 ? 'bg-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
            <div className={`w-2.5 h-2.5 rounded-full ${step >= 2 ? 'bg-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
            <div className={`w-8 h-1 rounded-full ${step >= 3 ? 'bg-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
            <div className={`w-2.5 h-2.5 rounded-full ${step >= 3 ? 'bg-primary-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
          </div>
          ...
          {step === 1 && (
            <form onSubmit={handleRequestOtp} className="space-y-4">
              <p className="text-center text-slate-600 dark:text-slate-400 text-sm mb-6 font-medium">
                আপনার রেজিস্ট্রিকৃত মোবাইল নাম্বার বা ইমেইল প্রদান করুন। আমরা একটি সিকিউরিটি কোড পাঠাবো।
              </p>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">মোবাইল নাম্বার বা ইমেইল</label>
              ...
              >
                {isForgotLoading ? "খোঁজা হচ্ছে..." : "কোড পাঠান"} <ArrowRight className="w-5 h-5" />
              </button>
            </form>
          )}
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div className="text-center mb-6">
                <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">
                  আমরা <strong>{identifier}</strong> নাম্বারে/ইমেইলে একটি ৬-সংখ্যার কোড পাঠিয়েছি।
                </p>
                <p className="text-xs text-primary-600 font-bold mt-2">(টেস্টিং এর জন্য ডামি OTP: 123456 ব্যবহার করুন)</p>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1 text-center">OTP ইনপুট করুন</label>
                <input 
                  type="text" 
                  required
                  maxLength={6}
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 mt-2 bg-primary-600 text-white font-bold rounded-xl shadow-md hover:bg-primary-700 transition disabled:opacity-70"
              >
                {isVerifyLoading ? "ভেরিফাই হচ্ছে..." : "যাচাই করুন"} <CheckCircle2 className="w-5 h-5" />
              </button>
              <button 
                type="button" 
                onClick={() => setStep(1)}
          ...
          {step === 3 && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <p className="text-center text-slate-600 dark:text-slate-400 text-sm mb-6 font-medium">
                আপনার পরিচয় সফলভাবে যাচাই করা হয়েছে। এখন নতুন একটি শক্তিশালী পাসওয়ার্ড সেট করুন।
              </p>
              <div>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">পুনরায় নতুন পাসওয়ার্ড</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-3.5 h-5 w-5 text-slate-400" />
                  <input 
                    type="password" 
              ...
              >
                {isResetLoading ? "সেভ হচ্ছে..." : "নতুন পাসওয়ার্ড কনফার্ম করুন"}
              </button>
            </form>
          )}

          <div className="mt-8 text-center border-t border-slate-100 dark:border-slate-700 pt-6">
            <Link href="/login" className="text-slate-500 hover:text-primary-600 dark:text-slate-400 dark:hover:text-primary-400 font-bold text-sm">
              লগইন পেজে ফিরে যান
            </Link>
          </div>
